...
import { Check, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Copy, DollarSign, Search, User, Users } from "lucide-react";
...
const getStatusPagamento = (inscrito: InscritoType) => {
    ...
    switch (inscrito.pagamento.status) {
        ...
        case 'credenciada': return "Credênciada"
        default: return "Aguardando"
    }
}

export default function CardTableCredenciamento({ celulas, evento, inscricoes }: Props) {
    ...
    return <div className="grid gap-6">
        <Card>
            <CardHeader>
                <CardTitle>Credênciamento</CardTitle>
                <CardDescription>
                    {inscricoesFiltradas.filter(f => getStatusPagamento(f) === "Credênciada").length} de {inscricoesFiltradas.filter(f => getStatusPagamento(f) === "Pago").length} inscrições credênciadas
                </CardDescription>
            </CardHeader>
            <CardContent>
                            ...
                            inscricoesFiltradas
                                ...
                                .map((inscrito, i) => <TableRow key={inscrito.cpf} className={i % 2 == 0 ? "bg-accent" : "null"}>
                                    ...
                                    <TableCell className="text-right flex space-x-2">
                                        {
                                            <DialogTableCredenciamentoCamera evento={evento} inscrito={inscrito} />
                                        }
                                    </TableCell>
                                </TableRow>)
                        }
